...
function Set-Config {
    ...
    #Asking the user for the Stash URL, with some error handling
        ...
        while ($null -eq $StashGQL_URL ) {
            ...
            $StashSecured = read-host "`nDoes your Stash have a username and password configured? [Y/N] (Default is 'No')"
            if (($StashSecured -like 'Y*')) {
                write-output "`nNote: Your API key will be stored in plain text"
                $StashApi_Key = read-host "`Please enter your API key"
            }
            ...
            try {
                Invoke-GraphQLQuery -Query $StashGQL_Query -Headers $(if ($StashApi_Key) { @{ApiKey = "$StashApi_Key" } } ) -Uri $StashGQL_URL | out-null
            }
            catch {
    ...
    if ($StashApi_Key) {
        write-output "API Key: `n - $StashApi_Key`n"
    }
    ...
    try { 
        ...
        if ($StashApi_Key) {
            Add-Content -path $PathToConfigFile -value "## API Key ##"
            Add-Content -path $PathToConfigFile -value $StashApi_Key
        }
    }
    catch {
...
function Add-MetadataUsingOFDB {
    ...
    if (($backupConfirmation -like "Y*")) {
        ...
        try {
            Invoke-GraphQLQuery -Query $StashGQL_Query -Headers $(if ($StashApi_Key) { @{ApiKey = "$StashApi_Key" } } ) -Uri $StashGQL_URL -Variables $StashGQL_QueryVariables | out-null
    ...
    try {
        $StashGQL_Result = Invoke-GraphQLQuery -Query $StashGQL_Query -Headers $(if ($StashApi_Key) { @{ApiKey = "$StashApi_Key" } } ) -Uri $StashGQL_URL -Variables $StashGQL_QueryVariables
    }
    catch {
    ...
    foreach ($currentdatabase in $OFDatabaseFilesCollection) {
        ...
        if ((!$SchemaIsValid)) {
            ...
            foreach ($OFDBMedia in $OFDBQueryResult) {
                ...
                try {
                    $StashGQL_Result = Invoke-GraphQLQuery -Query $StashGQL_Query -Headers $(if ($StashApi_Key) { @{ApiKey = "$StashApi_Key" } } ) -Uri $StashGQL_URL
                }
                catch {
                ...
                #Otherwise we have found a match! let's process the matching result and add the metadata we've found
                    ...
                    if (($StashGQL_Result.data.querySQL.rows.length -gt 1) -and ($searchspecificity -match "normal") ) {
                        ...
                        try {
                            $AlternativeStashGQL_Result = Invoke-GraphQLQuery -Query $StashGQL_Query -Headers $(if ($StashApi_Key) { @{ApiKey = "$StashApi_Key" } } ) -Uri $StashGQL_URL 
                        }
                        catch {
                    ...
                    if ($mediatype -eq "video") {
                        ...
                        try {
                            $DiscoveredPerformerIDFromStash = Invoke-GraphQLQuery -Query $StashGQL_Query -Headers $(if ($StashApi_Key) { @{ApiKey = "$StashApi_Key" } } ) -Uri $StashGQL_URL -Variables $StashGQL_QueryVariables
                        }
                        catch {
            ...
            if ($creatednewperformer) {
                ...
                try {
                    $performerimageURL = Invoke-GraphQLQuery -Query $StashGQL_Query -Headers $(if ($StashApi_Key) { @{ApiKey = "$StashApi_Key" } } ) -Uri $StashGQL_URL -Variables $performerimageURLVariables_GQLQuery
                ...
                if ($performerimageURL.data.findimages.images.length -ne 0) {
                    ...
                    try {
                        $performerimageURL = Invoke-GraphQLQuery -Query $StashGQL_Query -Headers $(if ($StashApi_Key) { @{ApiKey = "$StashApi_Key" } } ) -Uri $StashGQL_URL -Variables $UpdatePerformerImage_GQLVariables | out-null
                    }
                    catch {
...
$StashApi_Key = (Get-Content $pathtoconfigfile)[9]
...
if (($boolSetImageDetails[0] -eq "v0") -and ($boolSetImageDetails[1] -lt 24)) {
    #checking for 'v0' as I assume stash will go to version 1 at some point.

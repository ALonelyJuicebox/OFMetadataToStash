    ...
    foreach ($video in $OFDBQueryResult){
        ...
        if (!$StashDBQueryResult){
            ...
            if (Test-Path $OFDBfilename){
                write-host "`n### INFO ###`nThere's a file in this OnlyFans metadata db that we couldn't find the following file in your Stash.`nThat said, this file *is* on your filesystem.`nTry running a Scan task in Stash and then run this script again.`n`n - $OFDBfilename`n" -ForegroundColor Cyan
            ...
            #If the scene is in the OnlyFans DB but not on the filesystem, it might have been moved by the user. Prompt user to perform another OF scan
                ...
                write-host "`n### INFO ###`nThere's a file in this OnlyFans metadata db that we couldn't find the following file in your Stash.`nThis file also doesn't appear to be where the OnlyFans Metadata db thinks it should be.`nTry repopulating the OnlyFans metadata DB.`n`n - $OFDBfilename`n" -ForegroundColor Cyan
            }
        }
